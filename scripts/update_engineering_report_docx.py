...
def main() -> int:
    ...
    for text in [
        ...
        "在 80 个受控缺陷和 80 个负对照中，注册审核智能体得到真阳性 80、假阳性 0、假阴性 0、真阴性 80，分类准确率、精确率、召回率和 F1 值均为 100%，总体准确率的 Wilson 得分法 95% 置信区间为 97.66%—100%。随后将这 160 条冻结的 B 节点输出送入修订闭环：C Repair Proposal 节点针对 80 个问题记录生成 80 个原子补丁；D Independent Quality Gate 节点通过 79 个并阻断 1 个，最终应用并关闭 79 个问题，修复闭环成功率为 98.75%，Wilson 得分法 95% 置信区间为 93.25%—99.78%；80 个负对照全部保持不变。",
        "唯一被 D 节点阻断的补丁出现在主要结局时间范围字段。冻结登记原文将 approximately 拼写为 approximatly；注册审核智能体正确识别候选时间范围与权威证据冲突，却在建议值中自动纠正了来源拼写。该补丁在语义上接近权威内容，但不满足逐字保留冻结来源的严格追溯要求。D 节点因此返回 replacement_not_exactly_traceable_to_frozen_source，不应用补丁并保留原文，等待重新生成或人工确认。评测结束后没有修改提示词或重跑以消除该失败。",
    ...
    for text in [
        "当前受控实验已经完成确定性字段一致性检测，以及由真实 B Evidence Review 节点冻结输出驱动的 C Repair Proposal 原子修订和 D Independent Quality Gate 独立沙箱复检。该闭环在 80 个受控缺陷中应用并关闭 79 个修订，主动阻断 1 个不满足逐字追溯要求的补丁，且 80 个负对照均未改变。后续专业验证不作为当前受控指标成立的前置条件，而用于评价系统在自然发生的复杂临床语义、跨文档逻辑、统计策略和注册判断中的适用范围。",
