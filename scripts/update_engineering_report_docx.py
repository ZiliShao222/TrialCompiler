...
def main() -> int:
    ...
    for text in [
        ...
        "在 80 个受控缺陷和 80 个负对照中，注册审核智能体得到真阳性 80、假阳性 0、假阴性 0、真阴性 80，分类准确率、精确率、召回率和 F1 值均为 100%，总体准确率的 Wilson 得分法 95% 置信区间为 97.66%—100%。该智能体在 159/160 个场景中提取了与冻结登记逐字一致的候选修订值，字段值精确提取率为 99.38%；候选值只有进入 C 节点后才构成修订提案。",
        ...
        "上述结果证明 B 节点的注册审核智能体能够从结构化登记证据片段中提取指定字段，并识别受控的跨试验同字段错配；它不等同于对完整试验方案、统计分析计划、知情同意书或自然发生临床缺陷的专家级总体准确率。该智能体的输出仍属于候选问题记录，不能替代医学、统计、注册或质量人员的正式决定。",
    ]:
    ...
    for text in [
        "当前受控实验已经完成确定性字段一致性检测、B Evidence Review 节点注册审核智能体的语义复核、C Repair Proposal 节点的原子修订，以及 D Independent Quality Gate 节点的独立沙箱复检。后续专业验证不作为当前受控指标成立的前置条件，而用于评价系统在自然发生的复杂临床语义、跨文档逻辑、统计策略和注册判断中的适用范围。",
