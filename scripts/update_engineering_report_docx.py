...
def main() -> int:
    ...
    insert_body(
        heading_72,
        [
            "仓库已经实现事实和来源契约、依赖图、多角色审核、分阶段方案生成、确定性检查、B Evidence Review（证据审查与问题识别）节点的注册审核智能体语义复核、原子修订、沙箱复检、专业决策请求、审计记录、独立评分程序、命令行工具和应用程序编程接口。确定性路径处理数值、枚举值、标识符和旧值残留等可形式化问题；注册审核智能体从冻结注册证据片段中提取目标事实并形成带理由的问题记录。两类结果进入同一修订和复检流程，但语言模型不能修改权威事实、批准状态或发布门禁。",
    ...
    insert_body(
        ...
        [
            ...
            "输入可靠性由 50 份注册快照、65 份官方 PDF、文件摘要、页数和文本可提取性验证。任务有效性分别由官方文档元数据、400 个跨试验同字段错配缺陷与 400 个未修改负对照，以及 B Evidence Review 节点中注册审核智能体的真实服务接口盲测评价。修订安全性通过 400 个受控缺陷的原子补丁和 D Independent Quality Gate（独立沙箱复检）评价，检查权威值写入、错误值移除、修改局部性、来源保持、问题关闭和新增问题。",
            "确定性检查结果、注册审核智能体的语义复核结果和自然语言风险候选不合并为单一“系统准确率”。确定性实验衡量已编码字段约束，语义复核实验衡量注册审核智能体的结构化注册证据提取与一致性判断；自然临床语义仍需独立专业裁决。三者的输入难度、金标准形成方式和适用边界不同，必须分别报告。",
    ...
    for text in [
        ...
        "为评价 B Evidence Review（证据审查与问题识别）节点中的注册审核智能体能否从证据中独立定位字段，而非只执行代码级字符串规则，项目通过真实服务接口对该智能体开展盲测。其输入包含当前试验编号、目标字段名称、含 8 类字段的冻结注册证据片段和候选文档陈述，但不包含缺陷标签、缺陷来源案例或预设替换答案。注册审核智能体需要自行定位目标字段、提取权威值、判断候选内容是否一致，并在发现冲突时形成带来源定位的问题记录；C Repair Proposal（修订提案）节点据此生成候选补丁，最终修改再由 D Independent Quality Gate（独立沙箱复检）节点进行独立复检。",
        ...
        "在 80 个受控缺陷和 80 个负对照中，注册审核智能体得到真阳性 80、假阳性 0、假阴性 0、真阴性 80，分类准确率、精确率、召回率和 F1 值均为 100%，总体准确率的 Wilson 得分法 95% 置信区间为 97.66%—100%。随后将这 160 条冻结的 B 节点输出送入修订闭环：C Repair Proposal 节点针对 80 个问题记录生成 80 个原子补丁；D Independent Quality Gate 节点按字段类型和冻结表示逐条核查，通过 75 个并阻断 5 个，最终应用并关闭 75 个问题。严格修复闭环成功率为 93.75%，Wilson 得分法 95% 置信区间为 86.19%—97.30%；80 个负对照全部保持不变。",
        "5 个未通过补丁包括 1 个主要结局时间范围字段和 4 个研究臂数量字段。前者把冻结来源中的 approximatly 自动规范为 approximately；后四者分别把数值型冻结值 1 或 2 改写为带单位的字符串 1 arm 或 2 arms。它们在宽松语义比较下含义接近正确，但不满足结构化字段类型和冻结来源逐字表示要求，因此均由 D 节点返回 replacement_not_exactly_traceable_to_frozen_source，不应用补丁并保留修订前单元。评测结束后没有修改原始输出、调整提示词或重新请求模型以消除这些失败。",
        "上述结果同时验证了问题发现、修订提案和独立复检之间的职责分离：B 节点负责形成问题记录，C 节点负责生成候选补丁，D 节点依据冻结来源、修改局部性和来源链执行硬门禁；即使候选表达在语义上合理，也不能绕过逐字追溯要求。该结果不等同于对完整试验方案、统计分析计划、知情同意书或自然发生临床缺陷的专家级总体准确率，也不能替代医学、统计、注册或质量人员的正式决定。",
    ]:
        doc.add_paragraph(text)

    doc.add_heading("7.6 GitHub、Demo 与复现方式", level=2)
    for text in [
        "代码仓库地址为 https://github.com/ZiliShao222/TrialCompiler，完整修复闭环的冻结产物自提交 e15f3c6 起纳入仓库。复现应优先使用仓库内冻结输入和逐条记录，而不是先联网重新下载持续变化的注册数据。",
        ...
        "B 节点注册审核智能体的真实服务接口复现需从本地安全环境注入访问凭据，先以校准子集运行参数校准，再以独立测试集运行正式评测。访问凭据不写入仓库、配置文件、运行记录或审计日志。冻结输出保存逐条问题判断、字段提取、请求编号、延迟、令牌用量和汇总指标，并由独立评分程序读取；具体复现入口和产物索引见 GitHub 运行说明。",
        "完整修复闭环可执行：python scripts/evaluate_registered_agent_repair_loop.py。程序读取已经冻结的 B 节点输出，将其逐条送入 C Repair Proposal 和 D Independent Quality Gate；汇总报告与 160 条逐项记录位于 benchmarks/trialdocbench/public_corpus_050/registered_agent_repair_loop_v1/。",
        "由于 B 节点注册审核智能体调用的是服务端语言模型，冻结的结构化输出可以逐项复核，但未来重新请求不保证回答文字逐字相同。ClinicalTrials.gov 在线重建也应创建新语料版本，不得静默覆盖已经标注的冻结语料库。",
    ]:
        doc.add_paragraph(text)

    doc.add_heading("7.7 后续专业验证与扩围条件", level=2)
    for text in [
        "当前受控实验已经完成确定性字段一致性检测，以及由真实 B Evidence Review 节点冻结输出驱动的 C Repair Proposal 原子修订和 D Independent Quality Gate 独立沙箱复检。该闭环在 80 个受控缺陷中应用并关闭 75 个修订，主动阻断 5 个不满足字段类型或逐字追溯要求的补丁，且 80 个负对照均未改变。后续专业验证不作为当前受控指标成立的前置条件，而用于评价系统在自然发生的复杂临床语义、跨文档逻辑、统计策略和注册判断中的适用范围。",
