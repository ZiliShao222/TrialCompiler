...
OUTPUT = ROOT / "TrialCompiler_工科报告_6.4至结尾_Word版_修正版.docx"
...
def main() -> int:
    ...
    insert_body(
        ...
        [
            ...
            "验证材料只使用公开研究层资料或完全合成材料，不包含患者级数据。这里的“完成”表示研究原型与可复现实验链已经打通，不表示企业生产部署、法规认可或临床有效性验证已经完成。修正版第三轮相关回归测试已全部通过；完整测试数量以最终冻结提交的持续集成记录为准。",
        ],
    )
    ...
    for text in [
        ...
        "ClinicalTrials.gov 登记快照和公开附件在本轮受控一致性评测中作为冻结来源基线；这一评测约定用于确定字段传播问题的答案，不等同于美国国家医学图书馆、美国国立卫生研究院或监管机构对文件全部临床内容作出的专业批准。",
        "修正版受控缺陷覆盖入组人数、正式研究标题、研究总体状态、研究类型、主要结局指标、主要结局时间范围、疾病或研究条件以及研究臂数量等 8 类字段。每个案例的原始登记值始终作为权威事实；缺陷候选通过将另一项真实试验的同类字段值移植到当前案例的隔离文档副本中构造，并配置一条保持当前登记值的未修改负对照。由此形成 400 个受控缺陷和 400 个负对照，共 800 个评价场景。每条记录均保存当前案例编号、权威值、错误候选值、错误值来源案例、注册快照摘要和数据集划分。该设计不把人工变异值误作权威事实，也不把受控构造缺陷宣称为公开数据库中自然存在的错误。",
    ...
    for text in [
        "代码仓库地址为 https://github.com/ZiliShao222/TrialCompiler，修正版第三轮证据方向对应提交 cc69d9a。复现应优先使用仓库内冻结输入和逐条记录，而不是先联网重新下载持续变化的注册数据。",
        "确定性修正版可执行：python scripts/run_round3_rich_defect_benchmark.py。输出位于 benchmarks/trialdocbench/public_corpus_050/round3_rich_defects_v2/，包含 800 条逐项记录、汇总报告和设计说明。",
        ...
        "旧目录 round3_qwen_api/ 的第一版结果因错误地将人工变异值设为权威事实而被明确标记为已废弃，只保留审计，不得用于报告结论。由于 qwen-plus 是服务端模型，冻结 JSON 可以逐项复核，但未来在同名模型上重新请求不保证回答文字逐字相同。ClinicalTrials.gov 在线重建也应创建新语料版本，不得静默覆盖已经标注的冻结 corpus。",
    ]:
        doc.add_paragraph(text)

    doc.add_heading("7.7 后续专业验证与扩围条件", level=2)
    for text in [
        "当前受控实验已经完成字段一致性检测、Qwen 证据复核和原子修订闭环验证。下一阶段专业验证不作为本轮受控指标成立的前置条件，而用于评价系统在自然发生的复杂临床语义、跨文档逻辑、统计策略和注册判断中的适用范围。",
